...
const VideoIframe: React.FC<IProps> = (props) => {
    ...
    const handleChangeVideoWidth = useCallback(() => {
        ...
        const height = iframeRef.current ? iframeRef.current.offsetWidth * 0.4 : defaultHeight;
        return setVideoHeight(Math.floor(height * ratio));
    }, []);

    useEffect(() => {
        window.addEventListener('resize', handleChangeVideoWidth);
        const ratio = window.innerWidth > 990 ? 1.0 : window.innerWidth > 522 ? 1.0 : window.innerWidth > 400 ? 1.45 : 1.85;
        const height = iframeRef.current ? iframeRef.current.offsetWidth * 0.4 : defaultHeight;
