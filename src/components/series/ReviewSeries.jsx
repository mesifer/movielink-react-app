...
export default function ReviewSeries({ page, results, total_results, created_at }) {
    ...
    function bold(text) {
        var bold = /\*\*(.*?)\*\*/gm;
        var underline = /\_([^_><]+)\_/g;
        let preHTML = null;

        bold ? (preHTML = text.replace(bold, '<strong>$1</strong>')) : (preHTML = text);
        let html = preHTML.replace(underline, '<em>$1</em>');
                                        ...
                                        <div className="w-[100%] flex flex-col md:pt-0 pt-4">
                                            ...
                                            <div className="">
                                                <p className="font-light text-md md:text-md text-justify text-white/80">
                                                    {!toggle[index] ? (
                                                        <div
                                                            dangerouslySetInnerHTML={{ __html: `${bold(results[index].content.slice(0, 200))}` }}
                                                        ></div>
                                                    ) : (
                                                        <div dangerouslySetInnerHTML={{ __html: `${bold(results[index].content)}` }}></div>
                                                    )}
                                ...
                                <div key={index} className="flex md:flex-row flex-col gap-x-4">
                                    <div className="md:w-[12%]  md:gap-x-0 gap-x-4 md:gap-y-0  w-[100%] flex">
                                        <div
                                            className="w-[20%] md:w-[100%] md:h-[10vh] h-[9vh] bg-cover rounded-md bg-contain bg-center"
                                            style={{
                                                backgroundImage: `url(https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541)`,
                                            }}
                                        ></div>
                                        <div className="md:hidden text-xl mt-[-5px] font-bold text-green-500">{results[index].author}</div>
                                    </div>
                                    <div className="w-[100%] flex flex-col md:pt-0 pt-4">
                                        ...
                                        <div className="">
                                            <p className="font-light text-md md:text-md text-justify text-white/80">
                                                {!toggle[index] ? (
                                                    <div dangerouslySetInnerHTML={{ __html: `${bold(results[index].content.slice(0, 200))}` }}></div>
                                                ) : (
                                                    <div dangerouslySetInnerHTML={{ __html: `${bold(results[index].content)}` }}></div>
                                                )}
                                                <span onClick={() => toggleReadMore(index)} className="read-or-hide cursor-pointer text-white/50">
